...
async function transform(inputCss, tailwindCss, options) {
    ...
    function transformSingleVariant(variant, tailwindNormalized, inputNormalized) {
    ...
    const tailwindNormalized = normalizer1(await parseCss1(tailwindCss, options, true), options);
    const inputNormalized = normalizer2(await parseCss2(inputCss, options), options);
    ...
    const variantsMerged = Object.entries(inputNormalized)
        .flatMap(([variant, inputNormalized]) => {
            ...
            return transformSingleVariant(variant, singleVariantNormalized, inputNormalized);
        })
        .reduce((acc, curr) => {
    ...
    return Object.entries(variantsMerged).map(([baseSelector, results]) => {
