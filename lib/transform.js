const { parseColor, parseCss } = require('./parsers');
...
async function transform(inputCss, tailwindCss, options) {
    ...
    const tailwindNormalized = normalizer(await parseCss(tailwindCss), options);
    const inputNormalized = normalizer(await parseCss(inputCss), options);
