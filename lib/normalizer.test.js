...
test('normalizer should handle variants', async () => {
    const normalizedCss = normalizer(await parseCss(testData.cssWithVariants));
    expect(Object.keys(normalizedCss)).toMatchInlineSnapshot(`
        Array [
          "base",
          "hover",
          "focus",
    ...
    `);
    expect(normalizedCss).toMatchSnapshot();
});
